...
class GameModel: NSObject {
    ...
    func tileBelowHasSomeValue(loc: (Int, Int), _ value:Int) -> Bool {
        let (x,y) = loc
        if y == dimension-1 {
            return false
        }
    ...
    func userHasLost() -> Bool {
        ...
        for i in 0..<dimension {
            for j in 0..<dimension {
                ...
                case let .Tile(v):
                    if self.tileBelowHasSomeValue(loc: (i,j), v) || self.tileToRightHasSameValue(loc: (i,j), v) {
                    return false
                    }
                }
            }
    ...
    func merge(group: [TileObject]) -> [MoveOrder] {
        return convert(group: collapse(group: group))
    }
}
